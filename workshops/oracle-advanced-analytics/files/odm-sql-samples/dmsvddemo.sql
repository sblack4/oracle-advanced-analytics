Rem
Rem $Header: rdbms/demo/dmsvddemo.sql /main/8 2015/10/07 10:06:28 bmilenov Exp $
Rem
Rem dmsvddemo.sql
Rem
...
Rem
Rem    NAME
Rem      dmsvddemo.sql - Singular Value Decomposition Demo
Rem
Rem    DESCRIPTION
Rem      This demo illustrates SVD build and scoring in the context
REM      of two use cases.  
Rem
Rem    NOTES
Rem
Rem    MODIFIED   (MM/DD/YY)
Rem    bmilenov    09/27/15 - bug-21394151: view cleanup
Rem    mmcampos    07/01/15 - Enable model views
Rem    amozes      01/25/12 - updates for 12c
Rem    xbarr       01/10/12 - add feature_details demo
Rem    bmilenov    10/03/11 - Bug-10354925: use a single view for both use
Rem                           cases
Rem    bmilenov    11/30/10 - lrg-4947485: remove case id from PCA demo
Rem    bmilenov    10/13/10 - Edits based on code review by MMCAMPOS
Rem    bmilenov    10/07/10 - SVD algorithm demo
Rem    bmilenov    10/07/10 - Created
Rem
...
WITH
  s_mat AS (
  SELECT feature_id, value 
  FROM table(dbms_data_mining.get_model_details_svd('SVD_SH_SAMPLE', 'S'))
  WHERE feature_id<=5),
  v_mat AS (
  SELECT feature_id, attribute_name, value
  FROM table(dbms_data_mining.get_model_details_svd('SVD_SH_SAMPLE', 'V'))
  WHERE feature_id<=5),
  u_mat AS (
  SELECT feature_id, case_id, value
  FROM table(dbms_data_mining.get_model_details_svd('SVD_SH_SAMPLE', 'U'))
  WHERE feature_id<=5)
